...
class ControllerOCTemplatesMainOCTFunctions extends Controller {
    ...
    public function OctDateTime($array) {
        $data = array();
        $data = array_merge($data, $this->language->load('octemplates/oct_showcase'));

        //Розбиваємо дату
        $a = preg_split("/[^\d]/", $array[0]);
        $seconds = (isset($array[1]) && $array[1]) ? $array[1] : false;
        $today = date('Ymd');

        if (($a[0].$a[1].$a[2]) == $today) {
            //Якщо сьогодні
            return($data['text_today']." (".$a[3].":".$a[4].")");
        } else {
            $b = explode("-",date("Y-m-d"));
            $tom = date("Ymd",mktime(0,0,0,$b[1],$b[2]-1,$b[0]));
            $sec_time = (isset($seconds) && $seconds == true) ? " (".$a[3].":".$a[4].")" : "";

            if (($a[0].$a[1].$a[2])==$tom) {
                //Якщо вчора
                return($data['text_yesterday']."".$sec_time);
                //Якщо поточний рік, не виводимо Y
            }  elseif($a[0] == date('Y')) {
                $mm = intval($a[1]);
                return($a[2]." ".$data['text_month_'.$mm.'']."".$sec_time);
            } else {
                //Якщо пізніше виводимо Y
                $mm = intval($a[1]);
                return($a[2]." ".$data['text_month_'.$mm.'']." ".$a[0]."".$sec_time);
            }
        }
    }
}
