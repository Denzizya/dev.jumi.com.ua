...
//			"#input-mpn", "#input-isbn", 
	...
	els.each(function() {
		...
		if (elll.prev())
		{
			console.log(elll.prev());
		}
		if (elll.prev()
			&& elll.prev().prop("tagName")
			&& elll.prev().prop("tagName").toLowerCase() == "select"
			&& elll.prev().hasClass("attribute_select"))
		{
			elll = elll.prev();
			console.log(elll);
		}
		$( '<input type="button" class="translate_button" ' + customStyleAttr + ' title="Translate" onclick="doTranslateExpert(this)">' ).insertBefore( elll );
	});
}
...
function isTranslateExpertHTMLEditor(el) {
	return isTranslateExpertCKEDITOR(el) || isTranslateExpertSummernote(el) || isTranslateExpertSummernoteOld(el);
}
...
*/	
	...
	textToTranslateElement = textToTranslateElement.filter(function(){
		...
		.first();
	$(textToTranslateElement).attr('targetLangId', targetLangId);
...
function doTranslateExpert(event) {
	...
	if ($(el).prop("tagName")
		&& $(el).prop("tagName").toLowerCase() == "select"
		&& $(el).hasClass("attribute_select")
	)
	{
		el = btn.nextSibling.nextSibling;
	}
	...
	$.post(document.location.href.split("?")[0] + '?route=' + document.js_const_module_prefix + '/client_translate_expert/translate&user_token=' + getOcuser_token(),
		{ text: textToTranslate, source: source, target: $(textToTranslateElement).attr('targetLangId'), format: format }
	).done(function(data) {
		console.log(data);		
		if (!data.success)
		{
