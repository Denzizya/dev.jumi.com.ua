<?php
//oc40namespace Opencart\Admin\Controller\Extension\ClientTranslateExpertName\Module;
class ControllerExtensionModuleClientTranslateExpert extends Controller {
	...
	public function addScripts(&$route, &$args)
	{
		if ($this->config->get('module_client_translate_expert_status'))
		{
			$this->document->addScript($this->url->link('extension/module/client_translate_expert/getParamsJs', 'user_token=' . $this->session->data['user_token'] . '&t=' . time(), true));
			$this->document->addScript('view/javascript/client_translate_expert.js?v=' . $this->getCurrentVersion());
			$this->document->addStyle('view/stylesheet/client_translate_expert.css?v=' . $this->getCurrentVersion());
		}
	...
	public function getParamsJs()
	{
		$client_translate_expert_is_enabled = $this->config->get('module_client_translate_expert_status');	  
		$this->load->model('localisation/language');
		$client_translate_expert_languages = json_encode($this->model_localisation_language->getLanguages());
	...
	public function install()
	{
		...
		$this->model_setting_event->addEvent('module_client_translate_expert', 'admin/controller/common/header/before', 'extension/module/client_translate_expert/addScripts');
	}
	...
	public function uninstall()
	{
		...
		$this->load->model('setting/event');
    	$this->model_setting_event->deleteEvent('module_client_translate_expert');
	}
}
